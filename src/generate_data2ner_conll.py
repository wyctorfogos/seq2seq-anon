...
ENTITY_TYPES = {
    ...
    "ipaddress": "IP",
    # Placeholders que não são entidades, como user_id, podem ser omitidos ou mapeados para "O"
    "user_id": "O"
}
...
# Geradores Faker
# ======================
# Código completado e aprimorado
FAKER_GENERATORS = {
...
def generate_data():
    ...
    for i in tqdm(range(NUM_EXAMPLES), desc="Gerando exemplos"):
        ...
        # Encontra todos os placeholders únicos no template
        placeholders = sorted(list(set(re.findall(r'\{([^}]+)\}', template))), key=len, reverse=True)
        
        generated_values = {}
        for placeholder in placeholders:
            if placeholder in FAKER_GENERATORS:
                # Gera o valor uma única vez para cada placeholder
                generated_values[placeholder] = FAKER_GENERATORS[placeholder]()

        # Preenche o template com os valores gerados
        text = template.format(**generated_values)
        
        # Tokeniza o texto e obtém o mapeamento de offsets
        tokenized_output = tokenizer(
            text, 
            return_offsets_mapping=True,
            max_length=512, # Evita textos muito longos
            truncation=True
        )
        ...
        # Inicializa os rótulos com "O" (Outside)
        labels = ['O'] * len(tokens)
        
        for placeholder, value in generated_values.items():
            entity_type = ENTITY_TYPES.get(placeholder)
            
            # Pula se não for uma entidade a ser rotulada
            if not entity_type or entity_type == 'O':
                continue
            
            # Encontra todas as ocorrências do valor gerado no texto
            # re.escape lida com caracteres especiais que podem quebrar a regex
            for match in re.finditer(re.escape(str(value)), text):
                start_char, end_char = match.span()
                
                is_first_token = True
                for idx, (offset_start, offset_end) in enumerate(offsets):
                    # Se o token está dentro da span da entidade
        ...
        dataset.append({
            ...
            "tokens": tokens,
            "ner_tags_str": labels,
            "full_text": text # Opcional: para debug e análise
        })
